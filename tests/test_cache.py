...
@skipUnless(_redis_installed(), 'redis is not installed')
class RedisHashCacheTestCase(TestCase):
    def make_cache(self):
        return cache.RedisHashCache(key_prefix='gapi-python-client-test-case:')
    ...
    def test_get_set(self):
        c = self.make_cache()
        c.set('a', {'id': 1, 'foo': 'bar'})
        self.assertEquals(c.get('a', 1), {'id': '1', 'foo': 'bar'})

        c.set('a', {'id': 1, 'nest': {'with': 'eggs'}})
