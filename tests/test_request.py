...
class APIRequestorTestCase(unittest.TestCase):
    ...
    def test_get_resource_by_id(self, mock_uri_check, mock_request):
    ...
    @patch('gapipy.request.APIRequestor._get_uri_override')
    def test_get_with_falsy_resource_id_does_not_raise_error(self, mock_uri_check, mock_request):
        mock_uri_check.return_value = False
        requestor = APIRequestor(self.client, 'resources')
        requestor.get(0)
    ...
    @patch('gapipy.request.APIRequestor._get_uri_override')
    def test_list_resource(self, mock_uri_check, mock_request):
        mock_uri_check.return_value = False
        requestor = APIRequestor(self.client, 'resources')
        requestor.list_raw()
