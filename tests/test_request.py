...
class APIRequestorTestCase(unittest.TestCase):
    ...
    def test_get_resource_by_id(self, mock_request, mock_uri_check):
    ...
    def test_get_with_null_resource_id_and_uri_raises_error(self, mock_request, mock_uri_check):
        mock_uri_check.return_value = False
        requestor = APIRequestor(self.client, 'resources')
    ...
    def test_get_with_falsy_resource_id_does_not_raise_error(self, mock_request, mock_uri_check):
        mock_uri_check.return_value = False
        requestor = APIRequestor(self.client, 'resources')
        requestor.get(0)
        mock_request.assert_called_once_with('/resources/0', 'GET')

    def test_list_resource(self, mock_request, mock_uri_check):
        mock_uri_check.return_value = False
        requestor = APIRequestor(self.client, 'resources')
        requestor.list_raw()
        mock_request.assert_called_once_with('/resources', 'GET', params=None)

    def test_list_resource_with_parent(self, mock_request, mock_uri_check):
        mock_uri_check.return_value = False
    ...
    def test_list_generator(self, mock_list, mock_request, mock_uri_check):
        mock_uri_check.return_value = False
